...
export function FileItem({ name, size, state }: FileItemProps) {
  ...
  return (
    <div className={container()}>
      ...
      {state === 'error' ? (
        <div
          className="flex flex-1 flex-col items-start gap-1" /* flex-1 é para fazer o negócio ocupar o máximo de espaço que sobra com os outros elementos do código */
        >
      ...
      {state === 'complete' ? (
        <AiFillCheckCircle className="h-5 w-5 fill-violet-600 text-white" />
      ) : (
